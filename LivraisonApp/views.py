...
def livraison_list(request):
    ...
    return render(request, "livraisons_gerant.html", {"livraisons": livraisons})
def livraison_list_client(request):
    livraisons = Livraison.objects.all().order_by("-date_sortie")
    return render(request, "livraisons_client.html", {"livraisons": livraisons})
...
def livraison_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # statut par défaut géré par le modèle ("en_cours")
            # id_livreur, id_client restent null pour l'instant

            livraison.save()
            return redirect("livraison_list_client")  # vers la liste des livraisons
    else:
        form = LivraisonForm()
...
def livreur_livraisons(request, livreur_id):
    """Interface d'un livreur : voit uniquement ses livraisons."""
    livraisons = Livraison.objects.filter(id_livreur=livreur_id).order_by("-date_sortie")
    context = {
...
def livreur_livraisons(request, livreur_id):
    ...
    # Statuts disponibles pour la liste déroulante du livreur
    livreur_statuts = [
        ('en_cours', 'En cours'),
        ('livree', 'Livrée'),
        ('retour', 'Retour'),
